...
export default function PinDetail({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <button
        onClick={onBack}
        style={{ background: "transparent", border: "none", color: "var(--muted)", display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.5rem 0", cursor: "pointer", fontSize: "0.9rem", fontWeight: "600", marginBottom: "0.5rem", flexShrink: 0 }}
      ...
      {isConfirmingDelete ? (
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem", padding: "1rem 0" }}>
          <h2 style={{ fontSize: "1.25rem", margin: 0, fontWeight: 700, color: "var(--ink)" }}>Remove this memory?</h2>
          <p style={{ fontSize: "0.9rem", color: "var(--muted)", margin: 0, lineHeight: 1.5 }}>This will permanently remove &quot;{activePin.title}&quot; and all of its logs.</p>
          <div style={{ display: "flex", gap: "0.5rem" }}>
            <button className="glass-pill icon-button" style={{ flex: 1, background: "var(--paper-soft)", border: "1px solid var(--stone)", padding: "0.5rem 1rem", fontSize: "0.7rem", fontWeight: 700, display: "flex", justifyContent: "center", color: "var(--muted)" }} onClick={() => setIsConfirmingDelete(false)}>CANCEL</button>
        ...
        <div className="sidebar-scrollbar" style={{ overflowY: "auto", paddingRight: "0.5rem", flexGrow: 1 }}>
          <h3 style={{ fontSize: "0.85rem", color: "var(--muted)", textTransform: "uppercase", letterSpacing: "1px", marginTop: 0, marginBottom: "1rem" }}>Edit Memory Point</h3>
          ...
          <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.5rem" }}>
            ...
            <button className="glass-pill icon-button" style={{ flex: 1, background: "var(--paper-soft)", border: "1px solid var(--stone)", padding: "0.5rem 1rem", fontSize: "0.7rem", fontWeight: 700, display: "flex", justifyContent: "center", color: "var(--muted)" }} onClick={() => setIsEditingPin(false)}>CANCEL</button>
          </div>
        </div>
      ) : (
        <div className="sidebar-scrollbar" style={{ overflowY: "auto", paddingRight: "0.5rem", flexGrow: 1 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "0.5rem" }}>
            <div style={{ flex: 1 }}>
              ...
              <h2 style={{ fontSize: "1.5rem", margin: "0 0 0.5rem 0", fontWeight: 700, color: "var(--ink)", lineHeight: 1.2 }}>{activePin.location_name}</h2>
              <p style={{ fontSize: "0.95rem", color: "var(--muted)", margin: "0 0 1rem 0", lineHeight: 1.4 }}>{activePin.title}</p>
            </div>
            <button title="Edit pin" onClick={startEditingPin} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--muted)", flexShrink: 0, padding: "0.25rem", display: "flex" }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
            </button>
          </div>

          <h3 style={{ fontSize: "0.85rem", color: "var(--muted)", textTransform: "uppercase", letterSpacing: "1px", borderBottom: "1px solid var(--stone)", paddingBottom: "0.5rem", marginBottom: "1rem" }}>Log Entries</h3>
          ...
          {(activePin.logs || []).map((log, idx) => (
            editingLogId === log.id ? (
              ...
              <div key={log.id || idx} style={{ background: "var(--paper-soft)", border: "1px solid var(--stone)", padding: "1rem", borderRadius: "var(--radius-card)", marginBottom: "1rem" }}>
                <div style={{ fontWeight: 600, fontSize: "0.9rem", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.4rem", color: "var(--ink)", marginBottom: "0.5rem" }}>
                  ...
                  <button title="Edit log" onClick={() => startEditingLog(log)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--muted)", padding: 0, display: "flex" }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                  </button>
                  <button title="Delete log" onClick={() => handleDeleteLog(log.id)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "#B23A24", padding: 0, display: "flex" }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
                  </button>
                </div>
                <p style={{ fontSize: "0.85rem", margin: 0, whiteSpace: "pre-wrap", color: "var(--muted)", lineHeight: 1.6 }}>{log.content}</p>
              </div>
            )
          ))}

          {(activePin.logs || []).length === 0 && !isAddingLog && (
            <div style={{ fontSize: "0.9rem", color: "var(--muted)", fontStyle: "italic", textAlign: "center", padding: "1rem 0" }}>No logs recorded for this trip yet.</div>
          )}
          ...
          {isAddingLog ? (
            ...
            <button className="glass-pill" style={{ width: "100%", background: "rgba(0,0,0,0.04)", border: "1px dashed #cbd5e1", color: "var(--muted)", fontSize: "0.75rem", marginBottom: "1rem", padding: "0.5rem", cursor: "pointer" }} onClick={() => setIsAddingLog(true)}>+ ADD LOG ENTRY</button>
          )}
